...
public enum MountScript {
    ...
    public static let checkAndRepair = """
        ...
        dev="$(blkid -t TYPE=ntfs -o device | head -n 1)"
        [ -n "$dev" ] || exit 0

        if ntfsls -f "$dev" 2>/dev/null | grep -i hiberfil >/dev/null 2>&1; then
          echo "lukotta: Windows left this volume hibernated; not repairing it"
          exit 1
        fi
        ...
        # survives to the machine that ends up serving the drive, and the mount
        # log lives in a workspace deleted when the attempt ends. A repair on a
        # real drive therefore left no account of itself anywhere -- and the run
        # whose words were needed was the one nobody could read back. The volume
        # is the only place both sides can see, which is the same reason the
        # reclaim walk writes its log there.
        #
        # Written on every outcome, not only a clean one: a check that could not
        # finish is exactly the transcript worth keeping.
        record() {
        ...
        if command -v ntfsck >/dev/null 2>&1; then
          ...
          while [ $n -lt 4 ]; do
            n=$((n + 1))
            # The whole of it, not the last line.
            #
            # The last line says how many errors were left and nothing about
            # what they were, and ntfsck asks a question before each class of
            # repair -- so a run that declined one looks identical to a run
            # that had nothing to decline. On the owner's drive that difference
            # was the whole answer and it had been thrown away.
            ntfsck -a "$dev" > /tmp/lukotta-pass.out 2>&1
            { echo "=== pass $n ==="; cat /tmp/lukotta-pass.out; } \
              >> /tmp/lukotta-ntfsck.out
            sed "s/^/lukotta: ntfsck($n) /" /tmp/lukotta-pass.out
            out="$(tail -n 1 /tmp/lukotta-pass.out)"
            echo "lukotta: ntfsck pass $n: $out"
          ...
          case "$(ntfsck -n "$dev" 2>&1 | tail -n 1)" in
            Clean,*)
              ...
              record
              echo "lukotta: the volume is clean"
              exit 0
              ;;
          esac
          echo "lukotta: ntfsck could not bring this volume clean; trying ntfsfix"
          record
        fi

        ntfsfix -n "$dev" 2>&1 | grep "completed successfully" >/dev/null 2>&1 || exit 1
        ntfsfix -d "$dev" || true
        """

...
public enum MountScript {
    ...
    public static func microsoftActionsTOML(servingAFP: Bool) -> String {
        let share = servingAFP ? "; \(writeShareScript)" : ""
        let serve = servingAFP ? " sh \(shareScriptPath) >/dev/null 2>&1" : ""
        return """
    ...
    private static func repairAction(
        ...
        let repair = withRepair ? "\n" + microsoftActionsTOML(servingAFP: servingAFP) : ""
        return """
            __tune_setup() {

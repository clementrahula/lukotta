...
public enum MountScript {
    ...
    public struct Inputs {
        ...
        public var hiddenFromFinder = false
        /// Served over SMB from the guest instead, where macOS has no AFP client.
        public var servesSMB = false
    ...
    /// remove.
    /// The prefix every action this app generates is named with.
    ///
    /// The name used to be `lukotta` flat, one section shared by every drive,
    /// and that is a race with the drive's own data in it. A container's action
    /// carries device-specific paths -- its scratch directory, its bind mounts,
    /// its list of logical volumes -- and the mount writes the section, strips
    /// any section of the same name, and then mounts with `-a lukotta`. Two
    ...
    public static func generatedAction(driveName: String, devicePath: String) -> String {
        generatedActionPrefix + exportName(driveName: driveName, devicePath: devicePath)
    }
    ...
    public static func build(_ i: Inputs) -> String {
        ...
        lines.append(
            repairAction(
                ...
                servingAFP: i.hiddenFromFinder, servingSMB: i.servesSMB))
        ...
        var result = targets.flatMap { target in
            drivers.map { driver in
                ...
                if driver == "ntfs3" && !i.readOnly && action == tunedActionName {
                    ...
                    && action == tunedActionName
                {
                    // Only where there is no more important action already.
                    //
                    // Without that last condition this replaced the repair on
                    // the repair rung: the ntfs-3g attempt that runs after
                    // ntfsfix stopped running ntfsfix, so a dirty volume was
                    // handed to a driver that refuses dirty volumes. The walk
                    // is not worth losing the repair for -- it is carried by
    ...
    /// nothing at all.
    /// Why this is now a file rather than the three commands it used to be.
    ///
    /// Those three could clear a dirty flag and nothing else, and they refused
    /// every volume with real damage: `ntfsfix -n` does not print "completed
    /// successfully" for a broken index entry or an inconsistent bitmap, so the
    /// action exited non-zero and the drive was handed back read-only. That is
    /// the whole set of drives somebody actually needs help with, and the only
    /// remaining answer for them was to find a Windows PC and run chkdsk.
    ///
    /// The guest now carries `ntfsck`, so there is a real check to run, and the
    /// repair below runs it. It is a file for the same reason the reclaim walk
    ...
    public static let checkScriptPath = "/tmp/lukotta-check"

    /// Puts the check in the guest, as a file, before anything is mounted.
    public static var writeCheckScript: String {
        "echo \(checkEncoded) | base64 -d > \(checkScriptPath)"
    }
    ...
    /// WHEN IT RUNS
    ///
    /// On the repair rung and nowhere else, which the ladder reaches only after
    /// a plain writable mount has already failed. A volume that mounts is never
    /// scanned, so nothing pays for this but the drives that need it -- the same
    /// rule Windows uses, and the reason a full check is affordable here at all.
    ///
    /// WHY IT LOOPS
    ///
    /// One `ntfsck -a` does not finish the job and does not claim to. On the
    /// damage an interrupted copy leaves it declines its own mass-free -- "MFT
    /// scan is too unreliable to free a large number of clusters, refusing
    /// mass-free and marking them in-use instead" -- and stops with errors left
    /// and the dirty flag still set, which is a volume ntfs3 will not mount at
    /// all. With the structure now consistent the next pass applies the bitmap
    /// update it refused and reports clean. Measured, twice, identically: 5273
    /// of 5279 fixed on the first pass and the last one on the second; then the
    /// volume mounts, the folder is back and the orphans are in lost+found.
    /// Stopping at one pass reads as "cannot be repaired" one pass early.
    ///
    /// WHAT IT WILL NOT DO
    ///
    /// A hibernated volume is still refused, unchanged. Windows left a memory
    /// image describing state the disk does not have, and `ntfsck -f` would
    /// repair it by discarding that image -- which is the one thing here that
    ...
    /// WHERE IT FALLS BACK
    ///
    /// A build with no ntfsck vendored, or a volume ntfsck cannot bring clean,
    /// carries on into exactly the three commands this replaced. So this is
    /// never worse than what was here before, and on every drive with real
    /// damage it is the difference between a repair and a read-only mount.
    public static let checkAndRepair = """
        # Generated by Lukotta. Repairs an NTFS volume that would not mount.

        # Every NTFS volume this machine can see, not just the first one.
        #
        # `before_mount` is handed no variable naming the device -- the engine
        # exposes only ALFS_VM_MOUNT_POINT, and that not until after_mount -- so
        # the volume has to be found with blkid. Taking `head -n 1` of that is
        # right for a partition, which is what the app opens one of at a time,
        # and wrong for a container: every logical volume of a LUKS or BitLocker
        # container is served together, so a container with two NTFS volumes had
        ...
        check_one() {
          ...
          # so it must not run on every mount of a volume that is fine. The
          # volume itself says whether it wants one: the reclaim walk writes
          # "could not move:" when a name refuses even a rename, and that line
          ...
          # survives to the machine that ends up serving the drive, and the
          # mount log lives in a workspace deleted when the attempt ends. A
          # repair on a real drive therefore left no account of itself anywhere
          # -- and the run whose words were needed was the one nobody could read
          # back. The volume is the only place both sides can see, which is the
          # same reason the reclaim walk writes its log there.
          #
          ...
          if [ "$scan" = 1 ] && command -v ntfsck >/dev/null 2>&1; then
            ...
            while [ $n -lt 4 ]; do
              ...
              # Passes, not a pass.
              #
              # One `ntfsck -a` does not finish the job and does not claim to.
              # On the damage an interrupted copy leaves it declines its own
              # mass-free -- "MFT scan is too unreliable to free a large number
              # of clusters" -- and stops with errors left and the dirty flag
              # still set, which is a volume ntfs3 will not mount at all. With
              # the structure now consistent the next pass applies the bitmap
              # update it refused and reports clean. Measured twice,
              # identically: 5273 of 5279 fixed on the first pass and the last
              # one on the second.
              #
              # The whole of each pass is kept, not its last line: that line
              # says how many errors were left and nothing about what they
              # were, and ntfsck asks a question before each class of repair, so
              # a run that declined one looks identical to a run that had
              # nothing to decline.
              ntfsck -a "$dev" > /tmp/lukotta-pass.out 2>&1
              { echo "=== $dev pass $n ==="; cat /tmp/lukotta-pass.out; } \
                >> /tmp/lukotta-ntfsck.out
              sed "s|^|lukotta: ntfsck($n) |" /tmp/lukotta-pass.out
              out="$(tail -n 1 /tmp/lukotta-pass.out)"
            ...
            case "$(ntfsck -n "$dev" 2>&1 | tail -n 1)" in
              Clean,*)
                ...
                record
                echo "lukotta: $dev is clean"
                return 0
                ;;
            esac
            echo "lukotta: ntfsck could not bring $dev clean; trying ntfsfix"
            record
          fi

          # A hibernated volume is refused above and never reaches this. Salvage
          # is not passed either: it "may discard unrecoverable data", while
          # plain -a repairs the structure and keeps what it cannot place, which
        ...
        devs="$(blkid -t TYPE=ntfs -o device)"
        [ -n "$devs" ] || exit 0
        worst=0
        for d in $devs; do
          check_one "$d" || worst=1
    ...
    public static let shareScriptPath = "/tmp/lukotta-afp"

    // The mounted volume, also served over AFP by netatalk in the guest.
    public static let shareServe = """
        exec >/tmp/lukotta-afp.log 2>&1
        mkdir -p /run/lock /tmp/afpcnid
        mount -t tmpfs tmpfs /var/lib/netatalk
        ...
        netstat -ltn | grep -E ':(548|2049) '
        cat /tmp/afpd.log
        """

    public static var writeShareScript: String {
        "echo \(Data(shareServe.utf8).base64EncodedString()) | base64 -d > \(shareScriptPath)"
    }
    ...
    public static var writeSmbScript: String {
        "echo \(Data(smbServe.utf8).base64EncodedString()) | base64 -d > \(smbScriptPath)"
    }
    ...
    /// WHAT IT IS FOR
    ///
    /// ntfsfix clears the dirty flag and is not a chkdsk. What an interrupted
    /// copy leaves is a directory index entry whose MFT reference keeps a
    /// sequence number the record has moved past -- the kernel says
    /// "MFT: r=1b, expect seq=4 instead of 8!" -- and ntfs3 refuses it, rightly,
    /// because that check is what stops a handle resolving to whatever now
    /// occupies the record. Nothing here repairs it: not ntfsfix, not ntfs-3g,
    /// and the guest carries no checker.
    ///
    /// Measured: the folder cannot be read, cannot be deleted, and cannot be
    /// recreated. A person who pulls a drive out during a copy gets a folder
    /// they can never use again, and every later copy to that name fails --
    /// twelve of twelve in the harness, with "Invalid argument" inside the
    /// guest and "Stale NFS file handle" by the time Finder shows it.
    ///
    /// WHAT IT DOES
    ///
    /// Moves the entry aside. Measured on the real damage: after the rename the
    /// original name is usable again and files written into it arrive. Nothing
    /// is destroyed -- what was there is kept under the new name -- and the new
    /// name begins with a dot, which Finder hides, so the remains do not turn
    /// into a folder somebody has to wonder about. They cannot be deleted; that
    /// is written down rather than pretended away.
    ///
    /// WHY IT RUNS AFTER THE MOUNT AND IN THE BACKGROUND
    ///
    /// A scan before the volume is served is a person waiting, which is the one
    /// thing this must not cost. It runs after the mount, detached, so the drive
    /// appears at once and the names are freed behind it. It runs only on the
    /// repair rung -- a volume that came up clean has nothing to look for.
    ///
    /// HOW IT LOOKS
    ///
    /// A poisoned directory stats fine and refuses to be listed, so `ls` is the
    /// test, not `stat`; a poisoned file refuses to be statted at all, so a name
    /// the shell produced from a listing that then fails `-e` is one too. The
    /// walk is its own queue rather than find(1), which stops dead at the first
    /// bad entry instead of stepping over it, and it is bounded so a very large
    /// drive cannot spin in here.
    ///
    /// No apostrophes: this goes inside a single-quoted TOML value.
    public static let reclaimUnreadable =
        "M=\"$ALFS_VM_MOUNT_POINT\"; "
        + "[ -d \"$M\" ] || exit 0; "
        + "Q=/tmp/lukotta-walk; N=/tmp/lukotta-walk-next; "
        + "printf \"%s\\n\" \"$M\" > $Q; seen=0; "
        // Emptied at the start, so the log says what THIS walk found.
        //
        // It only ever appended, so a drive walked on every mount accumulated
        // the same lines over and over -- the owner's carried four identical
        // copies of the same five names. That is noise on somebody's volume,
        // and it also makes the file useless as an answer to "is there still
        // something here that cannot be freed", which is the question the probe
        // now asks it before deciding whether to check the volume.
        + ": > \"$M/.lukotta-reclaim.log\" 2>/dev/null || true; "
        // The check's transcript, where the volume is already mounted.
        //
        // On the ntfs-3g rung the check and the serving mount run in the same
        // machine, so what the check wrote to /tmp is still here. On the other
        // rungs it is not -- every mount attempt gets its own machine -- and
        // the check writes its own copy there instead. Either way the volume
        // ends up carrying an account of what was done to it.
        + "[ -s /tmp/lukotta-ntfsck.out ] && "
        + "cp /tmp/lukotta-ntfsck.out \"$M/.lukotta-check.log\" 2>/dev/null; "
        + "while [ -s $Q ] && [ $seen -lt \(reclaimDirectoryLimit) ]; do "
        + ": > $N; "
        ...
        // single failed read is not proof that an entry is broken -- a busy
        // moment, a request that timed out, a volume still settling. What is
        // being looked for does not heal: measured on a real drive, a poisoned
        // name refuses ls, rmdir and mkdir every time, minutes apart. So a
        // second refusal is required, and a first one that passes on the retry
        // is left alone.
    ...
    /// How many directories the reclaim walk will visit before it stops.
    ///
    /// A bound rather than a promise to finish: a drive with millions of
    /// directories must not leave a shell walking it forever, and the damage
    /// an interrupted copy leaves is where somebody was copying, which is near
    /// what they were doing rather than buried a hundred thousand folders deep.
    public static let reclaimDirectoryLimit = 20000

    /// Look before writing, and check the volume where it has asked to be
    /// checked.
    ///
    /// WHY THE PROBE IS WHERE THE QUESTION BELONGS
    ///
    /// The repair rung is reached only when a writable mount fails, which is
    /// the right rule for a volume left dirty and not the only way NTFS breaks.
    /// A drive interrupted during a copy and then unmounted cleanly carries
    /// broken directory index entries with no dirty flag to announce them: it
    /// mounts, the repair rung is never reached, and those names refuse ls,
    /// rm and mkdir for ever. Measured on a real 247 GB BitLocker drive that
    /// mounts perfectly and carries five of them.
    ///
    /// What already finds them is the reclaim walk, which runs after every
    /// writable NTFS mount and writes "could not move:" when a name refuses
    /// even to be renamed. That line is the volume asking for a check, and this
    ...
    /// Only where the volume will be handed to Finder over AFP.
    ///
    /// netatalk in the guest costs memory and a wait on port 548 at every
    /// mount, and on a Mac with no AFP client nothing will ever connect to it:
    /// the volume reaches Finder over the NFS mount instead. Serving it anyway
    /// was measured in this machine's own engine logs, on every writable NTFS
    /// mount, for a share nothing asked for.
    ///
    /// `true` is what macOS 26 generates, to the byte. That system cannot be
    /// run any more, so the text it produces is frozen rather than re-measured.
    public static func microsoftActionsTOML(servingAFP: Bool, servingSMB: Bool = false)
        -> String
    {
        let share =
            servingAFP
            ? "; \(writeShareScript)" : (servingSMB ? "; \(writeSmbScript)" : "")
        let serve =
            servingAFP
            ? " sh \(shareScriptPath) >/dev/null 2>&1"
            : (servingSMB ? " sh \(smbScriptPath) >/dev/null 2>&1" : "")
        return """
            [custom_actions.\(ntfs3ProbeActionName)]
            description = 'Generated by Lukotta; ntfs3 must pass a read-only probe first'
            \(guestEnvironment)
            before_mount = '\(nfsBlockSize); \(writeCheckedList); \(writeCheckScript); sh \(checkScriptPath); \(ntfs3Probe); \(writeReclaimScript)\(share)'
            after_mount = 'nohup sh \(reclaimScriptPath) >/dev/null 2>&1 &\(serve)'

            [custom_actions.\(repairActionName)]
            description = 'Generated by Lukotta; the same, and clears the dirty flag'
            \(guestEnvironment)
            before_mount = '\(nfsBlockSize); \(ntfsRepair); \(writeReclaimScript)\(share)'
            after_mount = 'nohup sh \(reclaimScriptPath) >/dev/null 2>&1 &\(serve)'

            [custom_actions.\(ntfs3gActionName)]
            description = 'Generated by Lukotta; frees names ntfs3 would not touch'
            \(guestEnvironment)
            before_mount = '\(nfsBlockSize); \(writeCheckedList); \(writeCheckScript); sh \(checkScriptPath); \(writeReclaimScript)\(share)'
            after_mount = 'nohup sh \(reclaimScriptPath) >/dev/null 2>&1 &\(serve)'
            """
    }

    private static func repairAction(
        configQ: String, actionQ: String, mergedQ: String, withRepair: Bool, servingAFP: Bool,
        servingSMB: Bool
    ) -> String {
        ...
        let repair =
            withRepair
            ? "\n" + microsoftActionsTOML(servingAFP: servingAFP, servingSMB: servingSMB) : ""
        return """
            __tune_setup() {
    ...
    private static func multiVolume(
        ...
        let action = generatedAction(driveName: i.driveName, devicePath: i.devicePath)
        ...
        return """
                awk -v s='\(scratch)' -v q="'" -v ro='\(perVolumeOptions(i))' -v act='\(action)' '\(volumeAction)' \(listQ) > \(actionQ)
                ...
                { awk 'BEGIN { skip = 0 }
                       /^\\[/ { skip = ($0 == "[custom_actions.\(action)]") }
                       !skip' \(configQ) 2>/dev/null; cat \(actionQ); } > \(mergedQ)
                cat \(mergedQ) > \(configQ)
                __first=$(printf '%s\\n' "$__lvs" | head -n 1)
                ALFS_PASSPHRASE="$__cred" \(engineQ) mount\(ownershipFlags(i))\(readOnlyFlags(i.readOnly)) -w false\(netHelperFlag(i)) --nfs-options=\(shellQuoted(nfsOptions(i))) -a \(action) "lvm:$__first" >> \(logQ) 2>&1
            """
    }
}

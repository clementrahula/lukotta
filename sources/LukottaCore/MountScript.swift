...
public enum MountScript {
    ...
    public static var microsoftActionsTOML: String {
        ...
        \(guestEnvironment)
        before_mount = '\(nfsBlockSize); \(writeCheckedList); \(writeCheckScript); sh \(checkScriptPath); \(ntfs3Probe); \(writeReclaimScript); \(writeSmbScript)'
        after_mount = 'nohup sh \(reclaimScriptPath) >/dev/null 2>&1 & nohup sh \(smbScriptPath) >/dev/null 2>&1 &'

        [custom_actions.\(repairActionName)]
        description = 'Generated by Lukotta; the same, and clears the dirty flag'
        \(guestEnvironment)
        before_mount = '\(nfsBlockSize); \(ntfsRepair); \(writeReclaimScript); \(writeSmbScript)'
        after_mount = 'nohup sh \(reclaimScriptPath) >/dev/null 2>&1 & nohup sh \(smbScriptPath) >/dev/null 2>&1 &'

        [custom_actions.\(ntfs3gActionName)]
        description = 'Generated by Lukotta; frees names ntfs3 would not touch'
        \(guestEnvironment)
        before_mount = '\(nfsBlockSize); \(writeCheckedList); \(writeCheckScript); sh \(checkScriptPath); \(writeReclaimScript); \(writeSmbScript)'
        after_mount = 'nohup sh \(reclaimScriptPath) >/dev/null 2>&1 & nohup sh \(smbScriptPath) >/dev/null 2>&1 &'
        """
    }

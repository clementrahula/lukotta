...
public enum MountScript {
    ...
    public static var microsoftActionsTOML: String {
        ...
        \(guestEnvironment)
        before_mount = '\(nfsBlockSize); \(writeCheckedList); \(writeCheckScript); sh \(checkScriptPath); \(ntfs3Probe); \(writeReclaimScript); \(writeShareScript)'
        after_mount = 'nohup sh \(reclaimScriptPath) >/dev/null 2>&1 & sh \(shareScriptPath) >/dev/null 2>&1'

        [custom_actions.\(repairActionName)]
        description = 'Generated by Lukotta; the same, and clears the dirty flag'
        \(guestEnvironment)
        before_mount = '\(nfsBlockSize); \(ntfsRepair); \(writeReclaimScript); \(writeShareScript)'
        after_mount = 'nohup sh \(reclaimScriptPath) >/dev/null 2>&1 & sh \(shareScriptPath) >/dev/null 2>&1'

        [custom_actions.\(ntfs3gActionName)]
        description = 'Generated by Lukotta; frees names ntfs3 would not touch'
        \(guestEnvironment)
        before_mount = '\(nfsBlockSize); \(writeCheckedList); \(writeCheckScript); sh \(checkScriptPath); \(writeReclaimScript); \(writeShareScript)'
        after_mount = 'nohup sh \(reclaimScriptPath) >/dev/null 2>&1 & sh \(shareScriptPath) >/dev/null 2>&1'
        """
    }
    ...
    static func driverOptions(_ driver: String?, readOnly: Bool = false) -> [String] {
        // A directory arrives 0755 owned by whoever opened the drive, and the AFP
        // server is not that user: creating or removing anything inside one is

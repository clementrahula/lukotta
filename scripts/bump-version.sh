...
case "$PART" in
  # The first number is the owner's decision, and stays one. They give it in
  # conversation and --approved records that; what this refuses is a major
  # version nobody asked for. It is never a question put back to them.
  major)
    [ "$APPROVED" = true ] || {
      echo "error: the first number is the owner's call. Say so, and pass --approved." >&2
      exit 1; }
    MA=$((MA+1)); MI=0; PA=0 ;;
  minor) MI=$((MI+1)); PA=0 ;;
  patch) PA=$((PA+1)) ;;

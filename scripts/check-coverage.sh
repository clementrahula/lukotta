...
# `|| true` and an emptiness check, where the caller count further down needs
# only the `|| true`. There, finding nothing is an answer -- no harness calls
# that script -- and the rule acts on it. Here, finding nothing means the
# pattern has moved, which is the refactor this rule exists to survive:
# rewording `name:` to `label:` makes the grep match nothing. Letting the
# failing pipeline end the run leaves the header printed and no verdict, and
# `|| true` alone would turn that silence green, with every rule below unrun.
# So zero is refused out loud.

...
w("")
w("FULocker itself is licensed **GPL-3.0-or-later** (see `LICENSE`). It embeds the")
...
w("Source for FULocker is at the project repository. For the embedded components,")
w("open an issue on the project repository, or write to the address in the")
w("repository README, and you will be sent the complete corresponding source for")
w("the exact versions shipped, for at least three years from the date of")
w("distribution, at no more than the cost of the transfer.")
w("")
w("## Host components")
w("")
...
if zfs:
    ...
    w("CDDL-licensed kernel modules with a GPL-2.0 kernel. FULocker never touches")

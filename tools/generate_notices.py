...
w("")
w("Lukotta is conveyed over a network, so source is provided under GPL-3.0")
w("section 6(d) and the equivalent paragraph of GPL-2.0 section 3: the complete")
w("corresponding source is offered **from the same place as the application, at")
w("no further charge**. Every release that carries a binary also carries the")
w("source for that binary and for every GPL component embedded in it.")
w("")
w("This is deliberately not the section 6(b) written-offer route. That option is")
w("written for object code \"embodied in a physical product\" and binds the")
w("distributor for three years; supplying the source alongside the download")
w("discharges the obligation at the moment of distribution instead.")
w("")
w("If a release is ever missing its source archive, that is a bug — write to")
w("**lukotta@rahula.dev** and it will be fixed.")
w("")
w("## Host components")
w("")
